...
#
# The fixing lives here rather than in .golangci.yml so that lint-golangci-lint.sh
# only reports. With `issues.fix` set in the config, a fixed issue is dropped
# from the report, so a lint run rewrites the files and then exits 0 with
# nothing to say.
#
# Usage:
...
if [[ $ret -ne 0 ]]; then
  echo "${out}" >&2
  a2t::log::warn 'Golangci-lint could not fix every issue.'
fi
